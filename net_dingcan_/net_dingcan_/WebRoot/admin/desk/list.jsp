						...
						<tr>
							...
							<td class="altbg1">
									<c:if test="${tmp.status ==1}">
										<font color="#DC143C">占用中</font>
									</c:if>
									<c:if test="${tmp.status ==0}">
										<font color="#006400">空闲中</font>
									</c:if>
							</td>
						
							<td class="altbg1">
									<a target="mainFrame"
										href="${pageContext.request.contextPath}/desk_edit.action?desk.id=${tmp.id }">修改
									</a>
									<a target="mainFrame"
										href="${pageContext.request.contextPath}/desk_delete.action?desk.id=${tmp.id }">删除
									</a>
							</td>
